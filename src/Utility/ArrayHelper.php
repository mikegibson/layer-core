<?php
/**
 * Hash Utility class for array manipulation
 * Copied from CakePHP
 *
...
class ArrayHelper {
	 ...
	 */
	public function get(array $data, $path, $default = null) {
	 ...
	 */
	public function extract(array $data, $path) {
		...
		// Simple paths.
		if (!preg_match('/[{\[]/', $path)) {
			return (array)$this->get($data, $path);
		}

		if (strpos($path, '[') === false) {
			$tokens = explode('.', $path);
		} else {
			$tokens = $this->app['string_helper']->tokenize($path, '.', '[', ']');
		}
	 ...
	 */
	public function insert(array $data, $path, $values = null) {
		...
		foreach ($data as $k => $v) {
			if ($this->_matchToken($k, $token)) {
				if ($conditions && $this->_matches($v, $conditions)) {
					$data[$k] = array_merge($v, $values);
					continue;
				}
				if (!$conditions) {
					$data[$k] = $this->insert($v, $nextPath, $values);
				}
			}
		}
	 ...
	 * @return array $data.
	 */
	protected function _simpleOp($op, $data, $path, $values = null) {
	 ...
	 */
	public function remove(array $data, $path) {
		...
		foreach ($data as $k => $v) {
			$match = $this->_matchToken($k, $token);
			if ($match && is_array($v)) {
				if ($conditions && $this->_matches($v, $conditions)) {
					unset($data[$k]);
					continue;
				}
				$data[$k] = $this->remove($v, $nextPath);
				if (empty($data[$k])) {
					unset($data[$k]);
				}
			} elseif ($match) {
	 ...
	 */
	public function combine(array $data, $keyPath, $valuePath = null, $groupPath = null) {
		...
		if (is_array($keyPath)) {
			...
			$keys = $this->format($data, $keyPath, $format);
		} else {
			$keys = $this->extract($data, $keyPath);
		}
		if (empty($keys)) {
			return array();
		}
		...
		if (!empty($valuePath) && is_array($valuePath)) {
			...
			$vals = $this->format($data, $valuePath, $format);
		} elseif (!empty($valuePath)) {
			$vals = $this->extract($data, $valuePath);
		}
		if (empty($vals)) {
		...
		if ($groupPath !== null) {
			$group = $this->extract($data, $groupPath);
			if (!empty($group)) {
				$c = count($keys);
				for ($i = 0; $i < $c; $i++) {
					if (!isset($group[$i])) {
	 ...
	 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/hash.html#Hash::format
	 */
	public function format(array $data, array $paths, $format) {
		...
		for ($i = 0; $i < $count; $i++) {
			$extracted[] = $this->extract($data, $paths[$i]);
		}
		$out = array();
		$data = $extracted;
		$count = count($data[0]);
	 ...
	 */
	public function contains(array $data, array $needle) {
	 ...
	 */
	public function check(array $data, $path) {

		$results = $this->extract($data, $path);
		if (!is_array($results)) {
			return false;
		}
	 ...
	 */
	public function filter(array $data, $callback = array('self', '_filter')) {

		foreach ($data as $k => $v) {
			if (is_array($v)) {
				$data[$k] = $this->filter($v, $callback);
			}
		}
	 ...
	 */
	public function flatten(array $data, $separator = '.') {
	 ...
	 */
	public function expand($data, $separator = '.') {
		...
		foreach ($data as $flat => $value) {
			...
			$result = $this->merge($result, $child);
		}
	 ...
	 */
	public function maxDimensions(array $data) {

		$depth = array();
		if (is_array($data) && reset($data) !== false) {
			foreach ($data as $value) {
				$depth[] = $this->dimensions((array)$value) + 1;
			}
		}
	 ...
	 */
	public function map(array $data, $path, $function) {

		$values = (array)$this->extract($data, $path);
	 ...
	 */
	public function reduce(array $data, $path, $function) {
	 ...
	 */
	public function apply(array $data, $path, $function) {
	 ...
	 */
	public function sort(array $data, $path, $dir, $type = 'regular') {
		...
		$sortValues = $this->extract($data, $path);
		...
		$result = $this->_squash($sortValues);
		$keys = $this->extract($result, '{n}.id');
		$values = $this->extract($result, '{n}.value');
	 ...
	 * @return array
	 */
	protected function _squash($data, $key = null) {
		...
		foreach ($data as $k => $r) {
			...
			if (is_array($r) && !empty($r)) {
				$stack = array_merge($stack, $this->_squash($r, $id));
			} else {
	 ...
	 */
	public function diff(array $data, $compare) {
	 ...
	 */
	public function mergeDiff(array $data, $compare) {
		...
		foreach ($compare as $key => $value) {
			if (!array_key_exists($key, $data)) {
				...
				$data[$key] = $this->mergeDiff($data[$key], $compare[$key]);
			}
		}
	 ...
	 */
	public function nest(array $data, $options = array()) {
		...
		$return = $idMap = array();
		$ids = $this->extract($data, $options['idPath']);
		...
		foreach ($data as $result) {
			...
			$id = $this->get($result, $idKeys);
			$parentId = $this->get($result, $parentKeys);
		...
		if ($options['root']) {
			...
			$root = $this->get($return[0], $parentKeys);
		}
