...
class StringHelper {
    ...
    public function insert($str, $data, $options = array()) {
        ...
        $data = (array)$data;
        if (empty($data)) {
            return ($options['clean']) ? $this->cleanInsert($str, $options) : $str;
        }
        ...
        switch ($clean['method']) {
            case 'html':
                ...
                if ($clean['andText']) {
                    ...
                    $str = $this->cleanInsert($str, $options);
                }
                break;
            case 'text':
                $clean = array_merge(array(
     ...
     */
    public function wrap($text, $options = array()) {
    ...
    public function excerpt($text, $phrase, $radius = 100, $ellipsis = '...', $charset = null) {
        ...
        if (empty($text) || empty($phrase)) {
            return $this->truncate($text, $radius * 2, compact('ellipsis', 'charset'));
        }
